...
from src.nn_guidance import u_2mean_safety, minimize_f_torch, transform_u, visualize_nn_reachset, make_simple_sfmap
...
def main():
    # Parameters
    rocket, _ = slr_config()
    x0 = np.array([0, 0, 1500, 5., 20., -30., np.log(rocket.mwet)])
    tgo = 60.
    dt = 1.
    N = int(tgo / dt)
    n_horizon = 1
    ...
    # Load safety map
    sfmap, _ = make_simple_sfmap(x_range=(-500, 500), y_range=(-500, 500), n_points=1000, dtype='float64')
    ...
    # Design variables
    var = Variable(torch.cat((U_.reshape(-1), torch.tensor([tgo_next_])), dim=0), requires_grad=True)

    # Parameters
    x0 = torch.from_numpy(x0).double().requires_grad_(True)
    tgo = torch.tensor(tgo).requires_grad_(True)
    sfmap = sfmap.requires_grad_(True)
    bounds = [(0., 1.) for _ in range(n_horizon * 3)] + [(0., 1.)]
    ...
    for i in range(1):
        ...
        visualize_resutls(var, x0, tgo, dt, rocket, sfmap, model, n_horizon=n_horizon, tgo_scale=tgo_scale)

        # solve
        var_opt = minimize_f_torch(f, var, bounds=bounds, disp=True, niter=10)

        visualize_resutls(var_opt, x0, tgo, dt, rocket, sfmap, model, n_horizon=n_horizon, tgo_scale=tgo_scale)



def visualize_resutls(var, x0, tgo, dt, rocket, sfmap, model, n_horizon=1, tgo_scale=100):
    U_ = var[:-1].reshape(n_horizon, 3)
    tgo_next_ = var[-1]
    tgo_next = tgo_next_ * tgo_scale
    print('x0: {}'.format(x0), 'tgo: {}'.format(tgo))
    mean_safety, reach_mask = u_2mean_safety(U_, tgo_next, x0, dt, rocket, sfmap, model, border_sharpness=10)
    print('Mean safety: {:.4f} | U_: {} | tgo_next: {}'.format(mean_safety.item(), U_, tgo_next.item()))
