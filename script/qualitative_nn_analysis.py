...
if __name__ == "__main__":
    ...
    out_dir = (
        f"../out/qualitative_analysis/{data_dtstring}_{model_dtstring}_{model_fname}"
    )
    ...
    for i, data in enumerate(tqdm(data_list)):
        ...
        poly = plt.Polygon(
            ...
            linewidth=0.5,
            edgecolor="k",
            label="Reachable Set Prediction",
            alpha=0.1,
            facecolor="r",
        )
        ax.add_patch(poly)
        ...
        ax.set_title(
            f"Alt: {x0[2]:.0f} m, V: ({x0[3]:.0f}, {x0[5]:.0f}) m/s, TGO: {tgo:.0f} s"
        )

        # save figure
        fig.savefig(
            os.path.join(out_dir, f"reachset_{i}.png"), bbox_inches="tight", dpi=300
        )
