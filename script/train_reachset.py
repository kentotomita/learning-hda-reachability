...
def main(device="cpu"):
    ...
    # load data
    _, _, reachset = read_reachset(
        "../out/20230803_021318/reachset.json", nmax=max_datapoints
    )
    ...
    for epoch in range(num_epochs):
        for i, data in enumerate(train_loader):
            ...
            if (
                torch.sum(feasible_data) > 0
            ):  # if there is at least one feasible reachset
                (
                    x_data,
                    y_data,
                    xmin_data,
                    ...
                    jump_at_xp,
                    params_pred,
                ) = feasible_reachset(data, nn_output)
            ...
            fov_radius = data[6]
            loss, feasible_loss, points_loss, bound_loss, jump_loss = compute_loss(
                x_data,
                y_data,
                xmin_data,
                xmax_data,
            ...
            if itr % save_itr == 0:
                torch.save(
                    model.state_dict(),
                    os.path.join(model_dir, "model_{}.pth".format(itr)),
                )
            ...
            running_loss_dict = {
                ...
                "feasibility": running_loss_dict.get("feasibility", 0)
                + feasible_loss.item(),
                "points": running_loss_dict.get("points", 0) + points_loss.item(),
                "bound": running_loss_dict.get("bound", 0)
                + bound_loss.item(),  # 'bound' is a reserved keyword in python, so we use 'jump' instead of 'bound
            ...
            if itr % print_itr == print_itr - 1:
                ...
                print(
                    "[{}, {}] loss: {:.3f}, time: {:.3f}".format(
                        epoch + 1,
                        i + 1,
                        running_loss_dict["total"] / print_itr,
                        elapsed,
                    )
                )
                print(
                    "  feasibility: {:.3f}".format(
                        running_loss_dict["feasibility"] / print_itr
                    )
                )
                print(
                    "  points: {:.3f}".format(running_loss_dict["points"] / print_itr)
                )
                ...
                # Tensorboard
                writer.add_scalar(
                    "Train/total-loss", running_loss_dict["total"] / print_itr, itr
                )
                for key, value in running_loss_dict.items():
                    writer.add_scalar(f"Train/{key}-loss", value / print_itr, itr)

                fig = visualize_prediction(
                    data, feasible_pred, params_pred, x_pred, y_pred, return_fig=True
                )
...
def visualize_prediction(
    data, feasible_pred, params_pred, x_pred, y_pred, idx=0, return_fig=False
):
    ...
    # add title
    ax.set_title(
        f"Feasibility score: {feasible_pred:.3f} | Ground truth: {feasible_data}"
    )
    ...
    ys, _ = reach_ellipses_torch(
        X=xs, param=(xmin_pred, xmax_pred, alpha_pred, yp_pred, a1_pred, a2_pred)
    )
    reach_pts[:n, 0] = xs
    reach_pts[:n, 1] = ys
    ...
    poly = plt.Polygon(
        reach_pts, fill=None, edgecolor="b", linestyle="--", label="Reachset"
    )
    ax.add_patch(poly)
...
def test_model(model, test_loader, writer, itr, device="cpu", num_test=1):
    ...
    with torch.no_grad():
        ...
        if writer is not None:
            # Tensorboard
            writer.add_scalar(
                "Val/total-loss", running_loss_dict["total"] / num_test, itr
            )
            for key, value in running_loss_dict.items():
...
def feasible_reachset(data, nn_output):
    ...
    params_pred = inverse_transform_reachsetparam(
        xmin_pred_, xmax_pred_, alpha_pred_, yp_pred_, a1_pred_, a2_pred_, alt
    )
