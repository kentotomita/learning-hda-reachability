    ...
    for i in tqdm(range(len(reachset))):
        ...
        # if there are some points on FOV circle, fit only left ellipse
            ...
            def func_b1(a1, xmin, x_first_on_fov_circle, y_first_on_fov_circle):
                xc = xmin + a1
                return np.sqrt(y_first_on_fov_circle**2 / (1 - (x_first_on_fov_circle - xc)**2 / a1**2))
            
            # fit left ellipse
            try:
                popt, _ = curve_fit(
                    f=lambda x, a1: reachset_model(x, a1, a2, func_b1(a1, xmin, rx[first_on_fov_circle], ry[first_on_fov_circle]), b2, xmin=xmin, xmax=xmax),
        ...
        # if there are no points on FOV circle other than x-max, fit both ellipses
            ...
            try:
                popt, _ = curve_fit(
                    f=lambda x, a1, a2, b1, b2: reachset_model(x, a1, a2, b1, b2, xmin=xmin, xmax=xmax),
                    xdata=rx, 
                    ydata=ry,
...
def reachset_model(X, a1, a2, b1, b2, xmin, xmax):
    ...
    C = -b1**2/a1**2*xc1**2 + b2**2/a2**2*xc2**2 + (b1**2 - b2**2)
    D = B**2 - 4*A*C
    if D < 0:  # no intersection
        ...
        if abs(A) < eps:
            if abs(xc1 - xc2) < eps:
                ...
                const = -(-b1**2/a1**2*xc1**2 + b2**2/a2**2*xc2**2) - (b1**2 - b2**2)
