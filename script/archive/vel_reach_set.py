...
def visualize_convex_hull(vertices):
    ...
    for simplex in hull.simplices:
        ...
        ax.add_collection3d(Poly3DCollection([polygon], alpha=.25, linewidths=0.5, edgecolors='k', facecolors='gray'))
...
def config():
    ...
    x0 = np.array([0., 0., 1500., 37.5, 0., -53., mwet])  # Initial state vector (m, m, m, m/s, m/s, m/s, kg)
...
if __name__=="__main__":
    ...
    x0_min = np.array([0., 0., 1500., 0., 0., -75, np.log(rocket.mwet * 0.9)]) 
    x0_max = np.array([0., 0., 1500., 37.5, 0., 0., np.log(rocket.mwet * 1.1)])
    ...
    # Feasible point
    n_per_step = 25
    c_list = [np.array([np.cos(theta), np.sin(theta)]) for theta in np.linspace(0, np.pi, n_per_step)]
    ...
    for i in range(n_max_steps):
        ...
        v1 = get_var(prob, np.array([0., 1.]))
        v2 = get_var(prob, np.array([1., 0.]))
        v3 = get_var(prob, np.array([-1., -1.]))
