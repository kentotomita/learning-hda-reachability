...
def fit_ellipse(reachset: np.ndarray, fov: float, nmin=3, ic_idx0=0):
    ...
    for i in range(len(reachset)):
        ...
        # check if there are less than nmin points; if so skip this IC
            ...
            if normalization:
                # normalization
                rx_norm = (rx - np.min(rx)) / (np.max(rx) - np.min(rx))
                ry_norm = ry / np.max(ry)
                xdata = rx_norm
                ydata = ry_norm
                p0 = [0.0, 1.0, 1.0]
            else:
                xdata = rx
                ydata = ry
                p0 = [(np.min(rx) + np.max(rx)) / 2, fov_radius, fov_radius]
            ...
            try:
                ...
                if debug:
                    print(f"IC {i} failed to fit ellipse with rx={rx}, ry={ry}")
                    fig, ax = plt.subplots()
                    ax.plot(rx, ry, "o")
                    plt.show()
                continue
            ...
            # denormalization
            if normalization:
                xc = xc * (np.max(rx) - np.min(rx)) + np.min(rx)
                a = a * (np.max(rx) - np.min(rx))
                b = b * np.max(ry)

            # store data
            out.append([ic_idx0 + i] + list(x0) + [tgo, xc, a, b])
