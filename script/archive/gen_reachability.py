...
if __name__=="__main__":
    ...
    c_list = np.array([[1., 0., 0.], [0., 1., 0.], [-1., 0., 0.]])
    ...
    for tfi, tf in enumerate(tf_list):
        for ci, c in enumerate(c_list):
            lcvx_obj = lc.LCvxMaxRange(rocket=rocket, N=N, parameterize_x0=True, parameterize_c=False)
            prob = lcvx_obj.problem(tf=tf, c=c)

            k = 0
            for vz in vz_list:
                for vx in vx_list:
                    x0 = np.array([0., 0., 1500., vx, 0, vz, np.log(1905.)])
                    lc.set_params(prob, {'x0': x0})

                    # solve problem
                    #prob.solve(verbose=False, requires_grad=True, eps_abs=1e-7, eps_rel=1e-7, eps_infeas=1e-8)
                    prob.solve(verbose=False, requires_grad=False, abstol=1e-9, reltol=1e-9, feastol=1e-8)
                    print(f'Problem parameters  tf: {tf: .1f}, c: {c}, vz: {vz: .1f}, vx: {vx: .1f} | problem status: {prob.status}, {prob.value}')
                    
                    # save results
                    fname = os.path.join('../out', f'tf_{tfi}_c_{ci}_{k}.json')
                    params = {'c': c.tolist(), 'gsa': gsa, 'pa': pa, 'vmax': vmax, 'Tmax': Tmax, 'Tmin': Tmin, 'mdry': mdry, 'mwet': mwet, 'g': g.tolist()}
