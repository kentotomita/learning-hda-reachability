...
def main():
    # Parameters
    rocket, N = slr_config()
    x0 = np.array([0, 0, 1500, 30.0, 0.0, -30.0, np.log(rocket.mwet)])
    tgo = 60.0
    dt = tgo / N
    ...
    spacing = (
        0.05 * (max(r[2]) - min(r[2])) / len(reach_sets)
    )  # This can be adjusted for optimal spacing.
    ...
    ax2.plot(
        x_range,
        np.tan(gsa) * np.abs(x_range - r[0, -1]),
        "k--",
        linewidth=0.5,
        alpha=0.2,
    )
    ax2.fill_between(
        x_range, np.tan(gsa) * np.abs(x_range - r[0, -1]), color="gray", alpha=0.5
    )
