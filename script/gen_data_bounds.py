"""Generate the bounds for data generation."""
import numpy as np
import cvxpy as cp
import sys
...
from config.softlanding_reachset import slr_config
...
def config():
    ...
    rocket.gsa = (
        5.0 * np.pi / 180
    )  # Use weak glide slope angle constraints (rad); from horizontal
    N = 100
    x0_min = np.array([0.0, 0.0, 1500.0, 20.0, 0.0, -30, np.log(rocket.mwet)])
    x0_max = np.array([0.0, 0.0, 1500.0, 30.0, 0.0, -30.0, np.log(rocket.mwet)])
    x0_bounds = (x0_min, x0_max)
    # tf_list = np.linspace(60, 80, 5)
    tf_list = [60.0]
...
def solve_mass_bound(rocket, N, tf, x0_bounds, steps, n_proc=1):
    ...
    if not n_proc == 1:
        ...
        with mp.Pool(n_proc) as p:
            ...
            results = []
            for result in tqdm(
                p.starmap(solve_mass_bound_k, params), total=len(params)
            ):
                results.extend(result)
            # """
...
def solve_vx_bound_k(maxk, rocket, N, tf, x0_bounds, v0_cstr, nc):
    ...
    out1 = solve_reach(lcvx, prob, np.array([1.0, 0.0]))
    out2 = solve_reach(lcvx, prob, np.array([-1.0, 0.0]))
    ...
    if out1 is None or out2 is None:
        ...
        if (
            v1[0, maxk] * v2[0, maxk] < 0
        ):  # V_horizontal = 0 is reachable; v1 and v2 are on different sides of the x-axis
            alt = (r1[2, maxk] + r2[2, maxk]) / 2
            tgo = tf - maxk * dt  # time to go
            vertices = [[0.0, alt, tgo]]
        else:
            vertices = []
    ...
    c_list = [
        np.array([np.cos(theta), np.sin(theta)]) for theta in np.linspace(0, np.pi, nc)
    ]
    for c in c_list:
...
def main(n_proc=1):
    ...
    # steps = [int(x) for x in np.linspace(1, N - 1, N -1)]
    ...
    for tf in tqdm(tf_list):
        ...
        vx_data.extend(
            solve_vx_bound(rocket, N, tf, x0_bounds, steps, nc=5, n_proc=n_proc)
        )
