...
if __name__ == "__main__":
    ...
    for data in reachset:
        ...
        if alt >= 250:
            ...
            if np.all(np.isnan(xy_coords)):
                ...
                xs = xmin + (xmax - xmin) / (xmax_true - xmin_true) * (
                    xy_coords[:, 0] - xmin_true
                )
                ys, _ = reach_ellipses_torch(
                    X=xs, param=(xmin, xmax, alpha, yp, a1, a2)
                )
                # convert to numpy
                xs = xs.detach().numpy()
                ys = ys.detach().numpy()
                ...
                points_error = np.linalg.norm(
                    xy_coords - np.hstack([xs.reshape(-1, 1), ys.reshape(-1, 1)]),
                    axis=1,
                )
    ...
    for feas_thresh in feas_thresholds:
        ...
        false_infeas.append(
            np.sum(np.logical_and(feas_preds_binary == False, feas_true == True))
            / n_infeas
        )
        false_feas.append(
            np.sum(np.logical_and(feas_preds_binary == True, feas_true == False))
            / n_infeas
        )
    ...
    ax.plot(
        feas_thresholds, false_infeas, label="False infeasible", c="k", linestyle="--"
    )
    ...
    xticks_labels = [
        f"{bin_edges[i]:.0f}m - {bin_edges[i+1]:.0f}m" for i in range(num_bins)
    ]
    ...
    # x-tick labels to represent the altitude ranges
    xticks_labels = [
        f"{bin_edges[i]:.0f}m/s - {bin_edges[i+1]:.0f}m/s" for i in range(num_bins)
    ]
