...
def main():
    ...
    for i in range(0, N, 5):
        ...
        mean_safety, sfmap_reachable_mask = ic2mean_safety(x0_i, tgo_i, model, sfmap, border_sharpness=10)
        ...
        sf = sfmap[::20, :]
        ax1.scatter(sf[:, 0], sf[:, 1], c=sfmap_reachable_mask, s=0.5, cmap='jet', alpha=0.5)
        ...
        sfmap = sfmap.numpy()
        sfmap = sfmap[::20, :]
        ax.scatter(sfmap[:, 0], sfmap[:, 1], c=sfmap[:, 2], s=0.5, cmap='jet', alpha=0.5)
        ...
        fov = plt.Circle((r[0, i], r[1, i]), fov_radius, fill=False, linewidth=1.5, linestyle='--')
        ax.add_patch(fov)
