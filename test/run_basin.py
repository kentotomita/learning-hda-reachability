...
def visualize_results(u_, tgo_next, x0, dt, sfmap, model, rocket):
    ...
    mean_safety, reach_mask = ic2mean_safety(
        x, tgo_next, model, sfmap, border_sharpness=1
    )
...
def main():
    ...
    rocket, _ = slr_config()
    x0 = torch.from_numpy(
        np.array([0, 0, 1500, 0.0, 30.0, -40.0, np.log(rocket.mwet)])
    ).double()
    tgo = 60.0
    dt = 1.0
    n_horizon = 1
    ...
    u_opt = basinhopping_torch(f, u_, bounds=[(0.0, 1.0)], niter=10, verbose=True)
