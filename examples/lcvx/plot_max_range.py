...
def config():
    ...
    tf = 55.
    dt = tf / N
    x0 = np.array([2000., 0., 1500., -10., 3., -75., np.log(rk.mwet)])  # Initial state vector (m, m, m, m/s, m/s, m/s, kg)
...
if __name__=="__main__":
    ...
    lcvx_obj = lc.LCvxMaxRange(rocket=rocket, N=N, parameterize_x0=False, parameterize_c=False)
    ...
    # Test parameterize c
    lcvx_obj = lc.LCvxMaxRange(rocket=rocket, N=N, parameterize_x0=False, parameterize_c=True)
    ...
    lcvx_obj = lc.LCvxMaxRange(rocket=rocket, N=N, parameterize_x0=True, parameterize_c=True)
