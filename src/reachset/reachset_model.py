...
def reach_ellipses_torch(X: torch.Tensor, param: torch.Tensor):
    ...
    Y_positive = torch.where(
        (xmin <= X) & (X < xp),
        torch.sqrt(torch.clamp(b1**2 * (1 - (X - xc1)**2/(a1**2 + eps)), min=eps)),
        0.
    )
    Y_positive = torch.where(
        (xp <= X) & (X <= xmax),
        torch.sqrt(torch.clamp(b2**2 * (1 - (X - xc2)**2/(a2**2 + eps)), min=eps)),
        Y_positive
    )
    Y_positive = torch.where(
        X > xmax,
    ...
    jump_at_xp = torch.sqrt(torch.clamp(
         (torch.sqrt(torch.clamp(b1**2 * (1 - (xp - xc1)**2/(a1**2 + eps)), min=eps)) 
          - torch.sqrt(torch.clamp(b2**2 * (1 - (xp - xc2)**2/(a2**2 + eps)), min=eps))
          ) ** 2, min=eps))
