...
def get_reachpoints(rocket, N, icset, c, ic_idx_list, c_idx, debug=False):
    ...
    for i in range(n_ic):
        ...
        rc = np.array([rc_x, 0.0, 0.0])
        if (
            tf != tf_before
        ):  # Update the problem only if tf is different from the previous one; saving compitation time
        ...
        if feasible:
            if not np.linalg.norm(rf[:2]) * np.tan(rocket.gsa) <= alt + 1e-3:
                print(
                    "GSA Cstr Failed: rf={}, alt={}, gsa={}".format(rf, alt, rocket.gsa)
                )
                rf = np.array([np.nan, np.nan, np.nan])
            if not fov_radius + 1e-3 >= np.linalg.norm(rf[:2]):
                print(
                    "FOV Cstr Failed: rf={}, fov_radius={}, x0: {}".format(
                        rf, fov_radius, x0
                    )
                )
                rf = np.array([np.nan, np.nan, np.nan])
...
def get_ic_list(rocket, N, x0_arr, tgo, debug=False):
    ...
    for i, tf in enumerate(tqdm(tgo)):
        if tf != tf_before:  # define problem only when tf changes
            prob = lcvx.problem(
                tf=tf, c=np.array([-1.0, 0.0, 0.0]), rc=np.array([0.0, 0.0, 0.0])
            )
