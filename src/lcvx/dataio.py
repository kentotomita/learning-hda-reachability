...
def save_results(prob:cp.Problem, lcvx_obj: LCvxProblem, tf: float, filename: str, params: dict = None):
...
def load_results(filename: str):
    ...
    if set(data.keys()) != set({'status', 'tf', 't', 'r', 'v', 'm', 'U'}):
        params = {key: value for key, value in data.items() if key not in {'status', 'tf', 't', 'r', 'v', 'm', 'U'}}
    else:
        params = None
