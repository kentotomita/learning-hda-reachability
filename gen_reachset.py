...
sys.path.append("../")
from src.reachset import get_reachpoints, save_reachset, read_icset, read_reachset
...
def main(
    ...
    cs = [np.array([np.cos(t), np.sin(t), 0.0]) for t in np.linspace(np.pi, 0.0, nc)]
    ...
    if n_proc > 1:
                ...
                if i + n_per_proc < n_ic:
                    args.append(
                        (
                            rocket,
                            N,
                            icset[i : i + n_per_proc],
                            c,
                            ic_idxset[i : i + n_per_proc],
                            c_idx,
                            debug,
                        )
                    )
        ...
        for c_idx, c in enumerate(cs):
            reachpoints.extend(
                get_reachpoints(rocket, N, icset, c, ic_idxset, c_idx, debug)
            )

    # ----------------- save -----------------
    save_reachset(
        rocket, N, icset_all, reachpoints, nc, outdir=datadir, fname="reachset.json"
    )
    _, _, reachset = read_reachset(os.path.join(datadir, "reachset.json"))

    n_infeasible = np.sum(
        [np.all(np.isnan(reachset[i]["rf"])) * nc for i in range(len(reachset))]
    )
    n_valid = np.sum(
        [np.sum(~np.isnan(reachset[i]["rf"])) / 3 for i in range(len(reachset))]
    )
